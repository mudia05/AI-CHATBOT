...
// So this is a new beginning, i want to try writing the whole javascript, sorry but modifying your own was quite confusing 
...
let chatHistory = [];
const maxHistoryLength = 4; // Keeps last 2 exchanges (user+assistant pairs)
...
async function fetchMistral(prompt) {
    ...
    try {
        ...
        return `Error: ${error.message || "Failed to get response from Mistral"}`;
    }
}
...
async function handleUserInput() {
    ...
    try {
        const botMessage = await fetchMistral(userMessage);
        typingIndicator.remove();
...
function clearChatHistory() {
  ...
  chatHistory = []; // Completely reset history
  addChatBubble("Chat cleared. Feel free to start again!", "bot");
}
...
function initializeChat() {
  ...
  // Initial greeting
  addChatBubble("Hello! How can I assist you today?", "bot");
}
